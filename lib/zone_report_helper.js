...
zone_report.controller('zone_reporter_table', function ($scope, $http) {
                            ...
                            for (i = 0; i < db_items.length; ++i) {
                                for (j = 0; j < previous_db_items.length; ++j) {
                                    ...
                                    if (db_items[i]["Zone"] === previous_db_items[j]["Zone"]) {
                                        ...
                                        db_items["ComponentsChange"] = getChangePercentage(db_items[i]["Components"], previous_db_items[j]["Components"]);
                                        ...
                                        db_items["BreakablesChange"] = getChangePercentage(db_items[i]["Breakables"], previous_db_items[j]["Breakables"]);
                                    }
                                }
                            }
                            });
    ...
    //$scope.test =  db_items[0]["P_SizeOnDisk"];
